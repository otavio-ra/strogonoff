...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=strogonoff.example.com
...
workon strogonoff  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development strogonoff npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop strogonoff
    docker rm strogonoff
    docker run --name strogonoff -d --env-file /home/ubuntu/strogonoff.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/strogonoff:/dkdata \
        strogonoff start_web.sh
}
...
echo_green "Welcome to strogonoff's dev env"
